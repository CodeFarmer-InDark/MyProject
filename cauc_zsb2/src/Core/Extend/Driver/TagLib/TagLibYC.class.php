...
class TagLibYC extends TagLib
{
    protected $tags   =  array(
        ...
        'getData'=>array('attr'=>'tag,name,model,condition,field,order,sql,limit,id,empty,key,mod,name','close'=>1),
        'getItem'=>array('attr'=>'tag,name,model,condition,field,sql,empty,name','close'=>1),
    );
	public function _getData($attr,$content)
	{
		...
		$field   = empty($tag['field']) ?'\'\'': '\''.addslashes($tag['field']).'\'';
		...
		if(!empty($model)){
			...
			if(empty($sql)){
				$parseStr  .= ' if(!isset('.$name.')) :'.$name.' = $'.$model.'Dao->Where($condition)->Field($field)->Order($order)->Limit($limit)->select();endif;';
			}else{
	...
	public function _getItem($attr,$content){
		...
		if(!empty($model)){
			...
			if(empty($sql)){
				$parseStr  .= ' if(!isset('.$name.')) :'.$name.' = $'.$model.'->Where($condition)->field($field)->find();endif;';
			}else{
