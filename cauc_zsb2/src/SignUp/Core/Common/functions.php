...
function U($url='',$vars='',$suffix=true,$domain=false) {
    // 解析URL
    //pathinfo模式添加index.php
//    preg_match('/[^\/]\w+\.php/',$_SERVER['REQUEST_URI'],$match);
//    $enter_file = $match[0];
//    if (!strpos($url,$enter_file)) $url = '/'.$enter_file.'/'.MODULE_NAME.'/'.$url;
...
/*
 * 获取当前环境最大上传文件大小
 * @param int $is_byte 是否以byte为单位显示，否则以配置中单位显示，默认1为是，0为否
 *
 * @return string
 */
function uploadMaxFilesize($is_byte=1){
    $upload_max_filesize = ini_get('file_uploads') ? ini_get('upload_max_filesize'): 0;
    if($is_byte){
