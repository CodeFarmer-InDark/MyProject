...
//        $data['runSystem'] = $getData['run_system'];
        ...
        $category = D('Category')->Where("module='Download' and parent_id!='0'")->Order('display_order DESC,id asc')->select();
        //取主ID下属分类
    ...
    private function getFileInfo($uploadFile){
        $val=array();
            $val['attach_file'] = formatAttachPath($uploadFile[0]['savepath'],'../Uploads/') . $uploadFile[0]['savename'];
            ...
            $val['extension']=pathinfo(UPLOAD_PATH.DIRECTORY_SEPARATOR.$val['attach_file'],PATHINFO_EXTENSION);
        return $val;
    }
    /**
     * 编辑
    ...
    public function doCommand()
    {
        ...
        switch ($operate){
            case 'delete':
                parent::_tagsDelete('download');
                parent::_delete('download', 0, array('attach_image', 'attach_thumb'));
                break;
            ...
            case 'update': parent::_batchModify('download', $_POST, array('title','display_order'));break;
