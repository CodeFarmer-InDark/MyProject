...
class PictureAction extends AdminAction
{
    ...
    {
        parent::_checkPermission('Picture_index');
    ...
    {
        parent::_checkPermission('Picture_insert');
        $this->display();
    }
    ...
    {
        parent::_checkPermission('Picture_modify');
    ...
    public function doDelete()
    {
        ...
        parent::_checkPermission('Picture_delete');
        parent::_setMethod('post');
        $item = intval($_POST['id']);
        empty($item) && parent::_message('error', '记录不存在');
    ...
    {
        parent::_checkPermission('Picture_command');
        if(getMethod() == 'get'){
        ...
        switch ($operate){
            case 'delete':
                parent::_delete('picture', 0, array('attach_file', 'attach_thumb'));
                break;
            ...
            case 'update': parent::_batchModify('picture', $_POST, array('title','display_order'), __URL__,'Picture', 'display_order DESC,id DESC');break;
