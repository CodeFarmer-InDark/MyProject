        ...
        if( !$ldap['bind'] ){
            ...
            if (false == $record) {
                ...
                if ($record['password'] != sysmd5($password)) {
                    // echo ('passwordFalse');
                    $data = array("msg" => 'passwordFalse', "url" => '');
                    echo json_encode($data);
                    exit();
                ...
                //dump($record['role_id']);
                //echo $roleDao->getLastSql();
                if (empty($getRole)) {
                    $data = array("msg" => 'roleLost', "url" => '');
                    echo json_encode($data);
                    exit();
                }
                ...
                if ($record['role_id'] != 1 || $record['role_id'] != 3){
                    $data = array("msg" => $loginSucces, "url" => '1');//组合成json格式数据
                }else{
