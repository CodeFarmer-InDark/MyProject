<?php
class AuthAction extends GlobalAction{
...
//        echo session('login_redirect_uri');die;
                ...
                if ($this->_varifyActivation($data['phone'],'mobile',$data['sms_code'])) {
                    if ($data['password'] == $data['opassword']){
                        ...
                        if (isCreditNo($data['id_number']) != false) {
                            ...
                            if(md5($data['verify_code']) == $_SESSION['verify']){
    ...
    public function sendSMS(){
        ...
        if ($data['type'] == 'forgotPassword'){
            ...
            if (empty($data['phone']) || empty($data['type']) || empty($data['verify_code']) || empty($data['sfzh']))
            {
...
//        }
        ...
        if(false == $res){
            $msg = '发送失败'.$data['phone'].$data['type'].$idCard;
        }else{
//            if ($res['error'] == 0) {
//                $code = 1;
//                //$msg = '发送成功';
