...
function build_app_dir() {
    ...
    if(is_writeable(APP_PATH)) {
        ...
        // 写入测试Action
        if(!is_file(LIB_PATH.'Action/DataAction.class.php'))
            build_first_action();
    }else{
        header('Content-Type:text/html; charset=utf-8');
...
function build_first_action() {
    ...
    file_put_contents(LIB_PATH.'Action/DataAction.class.php',$content);
}
...
function build_dir_secure($dirs=array()) {
    ...
    if(defined('BUILD_DIR_SECURE') && BUILD_DIR_SECURE) {
        defined('DIR_SECURE_FILENAME')  or define('DIR_SECURE_FILENAME',    'detail.html');
